...
getwd() # this returns a different value when executed within a RMD NB vs. script or command prompt
download.file(zip_url, 'data.zip')
unzip('data.zip', exdir = '.') # this will work when executing in a RMD notebook
# unzip('data.zip', exdir = 'cc-R-Names') # this will work when executing in an R script
```
...
########### actions from above to ensure we are in a state to continue #########
```{r load_packages}
library(readr)
library(dplyr)
library(ggplot2)
