...
use this option.
1. Use the *New Project* icon or command (under the *File* menu) to create a
new project. 
1. Using the *New Folder* button in RStudio's **Files** pane, create two
folders named 'data' and 'scripts'.
...
The dataset can be accessed different ways. For today's tutorial, we will
download the file directly to the project working directory that we
created above.
...
names2010[which.max(names2010$count),]

# Notice the trailing comma. 
# Think of bracket as slicing by [row(s), column(s)]
# So if we only want the overall most popular name,
# we don't even need to include the count.
names2010[which.max(names2010$count), 1] # 1 for the first column
```
...
# introducing a new operator - the margittr: %>%
# CTL+SHIFT+M
# explain what is a pipe - output of one command as input to another
names2011 %>% 
  filter(count == max(count))

# That is longer than our previous solution but much easier to read
